...
class Products extends Component {
  ...
  componentDidMount(){
    ...
    //this.pushToDatabase();
    this.removeMeatAndDishProducts(this.state.productsFromDatabaseClassification);
  }
  ...
  // Push products to database
  pushToDatabase = () => {
    
    var products = this.removeMeatAndDishProducts(this.state.productsFromDatabaseClassification);
 ...
 removeDuplicates = (originalArray, property) => {
  ...
  getProductsWithNutritionInfoWithoutMeatAndDishes = () => {
    let productsWithoutMeatAndDishesClassification = this.removeMeatAndDishProducts(this.state.productsFromDatabaseClassification);
    let allNutritionProducts = this.state.productsFromDatabaseNutrition;
    let trimmedArray = [];
    let values = [];
    let value;

    // Get id of every product
    for(let item of productsWithoutMeatAndDishesClassification){
      values.push(item.Nummer);
    }
    
    // Filter where id matches
    trimmedArray = allNutritionProducts.filter(item => {
      value = item.Nummer;
      // If value exist in our temporary array, return the object to the trimmed array
      if(values.includes(value)){
        return item;
      }
    });
